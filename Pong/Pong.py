...
- added sounds of collisions || sounddesign by EliasFIVE)
"""
...
while True:
    ...
    # Border chaking
    if ball.ycor() > 240:
        winsound.PlaySound('donk2.wav', winsound.SND_ASYNC)
        ball.sety(240)
        ball.dy *= -1

    if ball.ycor() < -240:
        winsound.PlaySound('donk3.wav', winsound.SND_ASYNC)
        ball.sety(-240)
        ball.dy *= -1
    
    if ball.xcor() > paddle_b.xcor()+50:
        winsound.PlaySound('grime.wav', winsound.SND_FILENAME|winsound.SND_NOWAIT)
        ball.goto(0,0)
        ball.dx *=-1
    ...
    if ((ball.xcor () > paddle_b.xcor() - 20
        ...
        and ball.ycor() > paddle_a.ycor() - 50)):
        winsound.PlaySound('donk1.wav', winsound.SND_ASYNC)
        ball.dx *= -1   
